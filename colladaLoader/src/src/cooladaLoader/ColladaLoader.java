...
public class ColladaLoader
{
    ...
    /**
     * this is the entry constructor to load a sketchup or serialized file.
     * there exist reserved suffixes: 
     * - '.dae' or '.kmz' are sketchup files
     * - '.dat' is a raw serialized ColladaLoader model (see method save())
     * @param the Filename (either 'foo.dae', 'foo.kmz' or 'foo.dat')
     * @param the main applet of processing (never null)
     */
    public ColladaLoader (String filename, PApplet applet)
    {
    	this.applet = applet;
        LoadingHelper helper = new LoadingHelper(filename, applet);
    ...
    /**
     * <p>A comfortable method to draw the complete dae/kmz to current scetch.
     * It's allowed to implement a custom code from the following template:</p>
     * <p><pre>
     * public void drawModel()
     *    {
     *    	//reset shapes
     *    	noStroke(); //reset if last draw was lines
     *    	fill(127); //define a standart color
     *    	
     *    	
     ...
     *       {
     *        	strokeWeight(1);//define thickness of drawn lines
     ...
     * </pre></p>
     */
//    public void draw(OpenGL11Adapter applet, OpenGL11Adapter PApplet) //needed on developers environment only
    public void draw()
    {
    	//reset shapes
        ...
        for (Triangle tri : triangles)
        {
            ...
            {
                applet.beginShape(PApplet.TRIANGLE);
//                applet.texture(tri.imageReference.width,tri.imageReference.height, tri.imageReference.pixels);//needed on developers environment only
        ...
        for (Line lin : lines)
        {
        	applet.strokeWeight(1);//define thickness of drawn lines
 ...
 */
    ...
    /**
     * this method saves the current model (and all its manipulations at runtime)
     * to the dataPath. The filename must end with ".dat" suffix
     * i.e. "myfile.dat"
     * @param fileNameEndsWithDat
     */
    public void save(String fileNameEndsWithDat)
    {
    	LoadingHelper.storeModel(fileNameEndsWithDat, this, applet);
    }
    /**
