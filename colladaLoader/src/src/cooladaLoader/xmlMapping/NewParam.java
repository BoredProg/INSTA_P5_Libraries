 ...
 */
 class NewParam extends SubTag{
       ...
       NewParam(XML newParam)
        {
            ID = newParam.getString("sid");

            XML initfrom = newParam.getChild("surface/init_from");
            if (initfrom != null) nextSource = initfrom.getContent();

            XML src = newParam.getChild("sampler2D/source");
            if (src != null) nextSource = src.getContent();
            
            if (ID==null || nextSource==null)
            	throw new ColladaLoaderException("newParam tag not parsed");
