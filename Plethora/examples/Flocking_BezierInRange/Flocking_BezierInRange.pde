...
void draw() {
  ...
  for (Ple_Agent pa : boids) {
    ...
    pa.flock(boids,   120, 40, 30 ,      1.5, 0.5, 1.5);
    ...
    //connect agents within range with a bezier calculated with the agents velocity as control point
    strokeWeight(1);
    stroke(0,90);
    //1 population, 2 distance from, 3 distance to, 4 scale bez 01, 5 scale bez 02
    pa.drawBezierInRange(boids, 10, 40 , 20, 20);
