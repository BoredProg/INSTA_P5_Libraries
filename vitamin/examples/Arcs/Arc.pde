...
//    _rootRib._leafTexID = _leafTex.getId();
...
/*      vgl.fill( 1 );
      for( int i=0; i<_path.size()-1; i++ )
      {
        ...
        Vector3 v1 = (Vector3)_path.get(i+1);
...
/*      // Save path to an array to be used below for the render
      Vector3[] buf = new Vector3[_path.size()];
...
//        _pathBuffer[_pathBuffer.length-2] = (Vector3)_path.get(_path.size()-1);
...
//    println( _depth + "  adding new child   " + _children.size() );
