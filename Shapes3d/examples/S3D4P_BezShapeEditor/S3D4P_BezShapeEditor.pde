...
public void editorMouseEventHandler(GWinApplet appc, GWinData data, MouseEvent event) {
  PVector vec;
...
synchronized public void drawEditor(GWinApplet wapp, GWinData windata) {
  while (mainDraw); 
  wapp.background(255);
  drawEdPaper(wapp);
  drawBezier(wapp);
...
public void drawEdPaper(GWinApplet wapp) {
...
public void drawBezier(GWinApplet wapp) {
  int nsteps = 20;
  int handleSize = 4;
...
public void makeControlWindow() {
  ...
  pnlControls = new GPanel(editor.papplet, 280, 20, 180, 80, "Controls");
  int h = 26;
  lblTexture = new GLabel(editor.papplet, 3, h, (int)pnlControls.getWidth() - 6, 14, "Texture");
  lblTexture.setTextAlign(GAlign.CENTER, GAlign.MIDDLE);
  pnlControls.addControl(lblTexture);

  btnPrevTexture = new GButton(editor.papplet, 10, h, 36, 16, "<<");
  pnlControls.addControl(btnPrevTexture);

  btnNextTexture = new GButton(editor.papplet, (int)pnlControls.getWidth() - 46, h, 36, 16, ">>");
  pnlControls.addControl(btnNextTexture);

  h = 42;
  lblZoom = new GLabel(editor.papplet, 3, h, (int)pnlControls.getWidth() - 6, 14, "Zoom 3D");
  lblZoom.setTextAlign(GAlign.CENTER, GAlign.MIDDLE);
  pnlControls.addControl(lblZoom);

  sdrZoom3D = new GSlider(editor.papplet, 10, h+12, (int)pnlControls.getWidth() - 20, 20, 12);
