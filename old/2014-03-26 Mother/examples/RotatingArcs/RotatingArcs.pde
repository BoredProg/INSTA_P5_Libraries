...
import javax.media.opengl.glu.*;
  ...
  void oscEvent(OscMessage theOscMessage)
  {
    try
    {
      ...
      {
        if (theOscMessage.checkTypetag("f")) /* check if the typetag is the right one. */
        {
      ...
      else if (theOscMessage.checkAddrPattern("/Alpha") == true)
      {
        if (theOscMessage.checkTypetag("i")) /* check if the typetag is the right one. */
        {
          m_Alpha.setValue(theOscMessage.get(0).intValue());
          return;
        }
      }
    } 
