...
//    final int Z = 2;
    static int A = 0; // Corresponds to three triangle points
    static int B = 1;
    static int C = 2;
    /*
    ...
    public boolean notDegenerateLine(float[][] coords) {
         return (Math.abs(coords[A][X] - coords[B][X]) > EPSILON) ? 
                true : (Math.abs(coords[A][Y] - coords[B][Y]) > EPSILON) ? 
                true : (Math.abs(coords[A][Z] - coords[B][Z]) > EPSILON);
    }
}
